...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    // Listen for postMessage events (e.g. from the Client ID config popup window)
    const messageListener = async (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;

      if (event.data && event.data.type === 'GOOGLE_CLIENT_ID_UPDATED') {
        const { clientId: newId } = event.data;
        if (isMounted) {
          setClientId(newId);
        }
      } else if (
            ...
            <div className="space-y-3">
              ...
              {clientId ? (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl py-3.5 text-xs font-bold transition-colors"
                >
                  ...
                  <span>Google Client ID 설정 필요</span>
                </button>
              )}
            </div>
