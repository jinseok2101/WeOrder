...
export function useRoomDetail() {
  ...
  const { messages, setMessages, orderTotals, setOrderTotals } =
    useRoomStore();

  const initialTab = (searchParams.get("tab") as "order" | "chat" | "settlement") || "order";
  ...
  useEffect(() => {
    const tabParam = searchParams.get("tab") as "order" | "chat" | "settlement";
  ...
  const handleDelete = () => {
    if (confirm("방을 삭제하시겠습니까? 방 안에 있는 모든 데이터가 사라집니다.")) {
